...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __getitem__(self, files_index):
        ...
        input_w, input_h = self.opt.img_size
        ...
        bbox_dist = np.zeros((output_h*output_w, 4), dtype=np.float32)
        centerness = np.zeros((output_h*output_w), dtype=np.float32)
        ...
        for k in range(num_objs):
            ...
            bbox = label[2:]
            cls_id = int(label[0])
            bbox[[0, 2]] = bbox[[0, 2]] * input_w
            bbox[[1, 3]] = bbox[[1, 3]] * input_h
            bbox[0] = np.clip(bbox[0], 0, input_w - 1)
            bbox[1] = np.clip(bbox[1], 0, input_h - 1)
            h = bbox[3]
            w = bbox[2]
            if h > 0 and w > 0:
                ct = np.array(
                ...
                for bboxj in range(left // self.opt.down_ratio, right // self.opt.down_ratio):
                    for bboxi in range(top // self.opt.down_ratio, bottom // self.opt.down_ratio):
                        ...
                        offset_x = (ct[0] - bboxj * self.opt.down_ratio) * 2 / (right-left)
                        offset_y = (ct[1] - bboxi * self.opt.down_ratio) * 2 / (bottom - top)
                        sigma = 1/3
                        centerness[flatten_idx] = np.exp(-(offset_x * offset_x + offset_y * offset_y) /
                                                         (2 * sigma * sigma))
