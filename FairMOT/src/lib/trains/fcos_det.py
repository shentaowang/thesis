...
class MotLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        opt = self.opt
        hm_loss, bbox_loss, centerness_loss = 0, 0, 0
        for s in range(opt.num_stacks):
            ...
            # flatten_cls_score = output['hm'].permute(0, 2, 3, 1).reshape(-1, self.opt.num_classes)
            # flatten_labels = batch['hm'].permute(0, 2, 3, 1).reshape(-1, self.opt.num_classes)
            # num_imgs = output['hm'].size(0)
            # num_pos = flatten_labels.eq(1).float().sum()
            # print(num_pos - flatten_labels.eq(1).float().sum())
            # print(num_pos, num_imgs)
            # hm_loss += self.crit(flatten_cls_score, flatten_labels, avg_factor=num_imgs + num_pos) / opt.num_stacks
        ...
        loss_stats = {'loss': loss, 'hm_loss': hm_loss,
                      'bbox_loss': bbox_loss, 'centerness_loss': centerness_loss}
...
class MotTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'bbox_loss', 'centerness_loss']
        loss = MotLoss(opt)
