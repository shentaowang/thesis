...
class FcosJDETracker(JDETracker):
    def __init__(self, opt, frame_rate=30):
        ...
        self.affine_id_last = -1
        self.image_affine_last = None
        self.affine_mat = np.eye(3, 3)
        self.min_match_count = 4
        self.affine_thre = 1.4

    def affine_estimate(self, img0):
        # init
        if self.image_affine_last is None:
            self.affine_mat = np.eye(3, 3)
            return

        orb = cv2.ORB_create()
        orb.setNLevels(1)
        if self.frame_id > self.affine_id_last + 1:
            image_affine_last = cv2.cvtColor(self.image_affine_last, cv2.COLOR_BGR2GRAY)
            self.kp_last, self.des_last = orb.detectAndCompute(image_affine_last, None)
            image_affine_cur = cv2.cvtColor(img0, cv2.COLOR_BGR2GRAY)
            self.kp_cur, self.des_cur = orb.detectAndCompute(image_affine_cur, None)
        else:
        ...
        self.affine_id_last = self.frame_id
