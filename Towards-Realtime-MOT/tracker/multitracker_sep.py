...
class JDETracker(object):
    def __init__(self, opt, frame_rate=30):
        ...
        self.model = Darknet(opt.cfg, nID=5273)
        ...
        print(self.det_thresh)

        self.kalman_filter = KalmanFilter()

        reid_path = '/home/sdb/wangshentao/myspace/thesis/code/Towards-Realtime-MOT/tracker/deep/checkpoint/ckpt.t7'
        self.extractor = Extractor(model_path=reid_path, use_cuda=True)
    ...
    def update(self, im_blob, img0):
        ...
        if len(pred) > 0:
            ...
            tlbrs = dets[:, :4]
            features = self._get_features(tlbrs, img0)
            detections = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f, 30) for
                          (tlbrs, f) in zip(dets[:, :5], features)]
        ...
        # cnt = 0
        # for item in strack_pool:
        #     if item.tracklet_len > 3:
        #         cnt += 1
        # print(cnt)
    ...
    def _get_features(self, tlbrs, ori_img):
        im_crops = []
        for bbox in tlbrs:
